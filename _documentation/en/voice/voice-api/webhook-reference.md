...
`detail` | `cannot_route` | Indicates the destination is not supported or blocked for the account (`cannot_route`), the number is not available (`number_out_of_service`) or a server error occured (`internal_error`)
`direction` | `outbound` | Call direction, this will be `outbound` in this context
`timestamp` | `2020-01-01T12:00:00.000Z` | Timestamp (ISO 8601 format)
