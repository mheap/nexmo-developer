---
title: Inbound SMS
description: How to receive SMS on your Vonage virtual numbers.
navigation_weight: 5
---

# Inbound SMS

To receive inbound SMS, create a [webhook endpoint](/concepts/guides/webhooks) and configure your account to use it in the API settings tab of the [Vonage Developer Dashboard](https://dashboard.nexmo.com/settings).

Any messages sent to one of your Vonage virtual numbers are sent to your webhook using a [GET] or [POST] request. When you receive a message on your webhook, you must send a `200 OK` or `204 No Content` response. If you don't, then Vonage will assume that you have not received the message and will keep resending it for the next 24 hours.
...
`messageId` | Vonage's unique identifier for this message.| Yes
`message-timestamp` | The [UTC±00:00](https://en.wikipedia.org/wiki/UTC%C2%B100:00) time when Vonage started to push this inbound message to your webhook endpoint, in the following format: `YYYY-MM-DD HH:MM:SS`.| Yes
